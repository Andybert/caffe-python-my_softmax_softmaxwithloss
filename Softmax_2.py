...
class SoftmaxLayer(caffe.Layer):
    ...
    def forward(self, bottom, top):
        ...
        for i in xrange(self.num):
            for j in xrange(self.dim):
                ...
                top_data[i, j] = np.exp(top_data[i, j]) + 1e-10
        for i in xrange(self.num):
            self.scale_data[i] = 0
            for j in xrange(self.dim):
    ...
    def backward(self, top, propagate_down, bottom):
        ...
        temp_diff = np.zeros((self.num, self.dim), dtype=np.float32)
        for m in xrange(self.num):
            for n in xrange(self.dim):
                ...
                for i in xrange(self.num):
                    for j in xrange(self.dim):
                        if (i == m and j == n):
                            temp_diff[i, j] = top[0].data[m, n] - top[0].data[i, j] * top[0].data[m, n]
                        else:
                            temp_diff[i, j] = 0 - top[0].data[i, j] * top[0].data[m, n]
                        bottom_diff[m, n] += top[0].diff[i, j] * temp_diff[i, j]
